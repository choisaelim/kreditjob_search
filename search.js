...
const findCompanyDetails = async (searchCompName, index, driver, searchItems) => {
    try {
        let result = {
            status: SEARCH_BLOCK,
        };
        ...
        if (searchItems.length > 1 && isNaN(index)) {
            ...
            if (searchItems.length == 1 || !isNaN(index)) {
                ...
                const detailsXpath = '//*[@id="summary"]/div[2]/div/div[1]/div[1]/span';
                const salaryXpath =
                ...
                await driver.wait(until.elementLocated(By.xpath(detailsXpath)), 3000);
...
const findCompanyList = async (searchCompName, index) => {
    try {
        ...
        if (searchInputs.length > 0) {
            ...
            return await findCompanyDetails(searchCompName, index, driver, searchItems);
        }
    } catch (error) {
        driver.close();
        return result;
